...
monthDeaths2020A1 <- week2020Deaths %>% 
  group_by(año,mes,codigoCA,ca,sexo,grEdad) %>% 
  summarize(numDefun=sum(numDefun)) %>% 
  ungroup() 

save(monthDeaths2020A1,file="rdata/monthDeaths2020A1.rdata")
...
monthDeaths2020A2 <- pmap_dfr(week2020Deaths,ndd)
save(monthDeaths2020A2,file="monthDeaths2020A2.rdata")
```


# Datos de mortalidad 1980-2020

Por último, combino los datos de mortalidad desde 1980 hasta 2021 (los datos de 2020 y 2021 se han obtenido a partir de los datos semanales y son provisionales)

```{r}
defunciones80_19 <- defunciones80_19 %>% 
  mutate(sexo=as.character(sexo))
morta2021Mes <- morta1921Mes %>% filter(año>2019)
defunciones80_21 <- bind_rows(defunciones80_19,morta2021Mes) %>% 
  arrange(año,mes,ca)
save(defunciones80_21,file="rdata/defunciones80_21.rdata")
```


```{r}
porMes <- defunciones80_21 %>% group_by(año,mes) %>% 
  summarize(numDefun=sum(numDefun)) %>% 
  mutate(fecha=as.Date(paste(año,mes,15,sep="-")))
ggplot(porMes,aes(x=fecha,y=numDefun)) + geom_line()
```
